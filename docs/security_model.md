...
Planned scope: localhost archive export／restore UI, remote publication adapters, and a production application boundary<br>
...
flowchart LR
    ...
    Artifact --> CAS
    Generic["Trusted sequential artifact workflow<br/>fresh attempt / one active review"] --> Artifact
    Generic --> APP
    Approval["Host Authenticator + project ACL<br/>opaque one-shot Decision approval"] --> Generic
    Binding["Trusted durable Proposal / Decision binding"] --> APP
    Journal[("Separate SQLite artifact journal<br/>pre-CAS intents + verified outcome")] --> Durable["Explicit durable orchestration<br/>auth / reconcile / fresh authority"]
    Durable --> Generic
    Generic --> Checkout["Bounded Decision checkout<br/>protected control + selected site"]
    ...
    class U,A,CLI,Request,Control,AUTH,Collector,Generic,Approval,Durable,Binding,Journal,Checkout,Proj boundary;
```
...
restart後のlocal generic reviewでは、public `ReviewId` lookupより前にfresh credentialとtrusted project configで
authentication／ACLを行う。durable orchestrationはjournalのserver-owned `DurableProposalBinding`、manifest digest、
exact intentをimmutable Proposal graph、一つのlive Ref snapshot、reflogへ照合し、新しいApplication instanceで
Human profile／ordinary one-shot registrationを構築する。旧credential、`AdmittedProposalHandle`、approval、
registration、permitを復元せず、bindingやReviewIdもbearer authorityではない。最終publicationは通常どおり
`HumanDecisionRuntime`のfull validation／CASを通り、terminal outcomeはselected-site bounded checkout後だけjournalへ
確定する。ObjectStore／RefStoreとjournalのtransactionはatomicでないため、Proposal／Decision CAS前のexact intentと
live reconciliationでtimeout／crash windowを収束させる。このlocal boundaryはtrusted config／journalが改竄されて
いない前提で、元Proposalのruntime capability intersectionを暗号学的に証明せず、durable ACLやmulti-process
linearizabilityも提供しない。
...
保存するがmapper単体はRefを更新しない。trusted workflowはprofile-owned repositoryをbootstrapし、current exact
Decision headからdeterministic fresh attempt Refへ一つのactive Proposalを作り、completed Decisionのselected siteと
次accepted manifestのbyte/path identityを検査して連続reviewを許可する。workflow内credential、identity、path、
profile、candidateはtrusted process configurationであり、OS user認証やbrowser requestではない。
`PendingArtifactProposal`とapprovalはnon-serializableかつone-shotである。durable APIはそれらを保存せず、journal
intentとlive graphからfresh authorityを作る。Rust trusted valuesもwire/browser authorityではない。v1 type／workflowは
caller-supplied attributionだけを許可し、executionを常に未検証とする。verified executor mode、HTTP／CLI／model
invocation、automatic worker、production orchestrationは現時点で未実装である。Creator Pilot／localhost UIは
...
現在の CLI からこれらを変更できない。creator input、二つのfsck profile、archive listing／export／restore profileは
localhost transport inputから変更できないserver-fixed ceilingである。maintenance／archive listing／export／restore profileは
...
server-owned `ReviewBinding`、bounded `ReviewState`、Proposal／Decision intent、verified Decision outcomeを保存する。
Proposalはprivate intentをCAS前に登録し、ordinary Application/Core publicationのexact receiptとlive bindingを検証した
後だけpublic `ReviewId`へfinalizeする。Decisionはdisposition、selected snapshot、reviewed manifest digest、expected/new
Decision head、feedback OIDをCAS前にexact intentへ固定し、Core receiptとbounded checkoutが一致した後だけoutcomeと
`decision_committed`を一transactionで確定する。同じidempotency key／request fingerprint／bindingのreplayは同じ
private intent、locator、またはoutcomeへ収束し、一値でも異なればconflictにする。

raw idempotency keyとcanonical requestは別domainのSHA-256へ変換し、credential、approval、permit、
Actor／Policy／Grant OID、repository path、raw key、rationale、canonical request bytesは保存しない。legacy rowを
新しいverified outcomeへ暗黙昇格しない。journal transactionとObjectStore／RefStore transactionはatomicではないため、
上位durable orchestrationだけがtrusted configとlive immutable graph／Ref／reflogを検査し、CAS未実行を証明したexact
intentをretryableへ戻すか、CAS済みのexact outcomeをcheckout後にcommitする。`ReviewId`、state、intent、outcome rowを
単独でauthentication／authorization／portable permit／Core admission proofとして扱わない。
...
作り、digest、Ref／head／OID、repository locator、Decision receipt、selected siteを含めない。application metadataは
strict `ReviewedPublicTargetV1` allowlistに限定し、private rationale、DOM locator、prompt／provider response、raw path、
authority identityを受けない。detached canonical projectionからescaped Markdown／script-free HTML／fixed manifest・
checksumとlocal Synapse／GitHub layoutを作るだけで、Git／credential／network inputを持たない。generator labelは
manifestとprojectionで一致し安全なversion文字列でなければならず、renderer profile/version自体はfixed v1 dispatchで
検証する。remote adapter、Git provenance、identity mapping、GitHub App、hosted serviceは未実装である。
...
| artifact review locator / journal | `ReviewId`は認可後のlookup locatorで、credential、capability、permit、Core receiptではない。journalはRef／ObjectStoreとatomicでない。durable orchestrationがauth／ACL後にtrusted configとlive graph／Ref／reflog／checkoutを検査してreconcileするが、portable authority、automatic worker、multi-process transactionではない |
| generic artifact workflow | mapper単体はRefを更新しない。trusted sequential workflowはfresh attempt／one-active-review／verified accepted baseをApplication／Coreへ通し、host approvalとexplicit local restart recoveryを持つ。real identity provider、host collector、HTTP／CLI／model invocation、generic browser UI、durable ACL、production deployment、tagged distributionは未実装 |
