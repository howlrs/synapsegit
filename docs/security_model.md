...
Implemented scope: local object / Ref / archive integrity path, process-local authenticated Creative AI and narrow Human Decision routes, Core admissions, provider-neutral bounded regular-file mapping and one same-process generic Proposal/Decision workflow, checked Human review re-registration from a trusted durable binding, separate SQLite artifact review/idempotency primitives, disposable SQLite projection libraries, read-only local publication bundles, and the single-user IPv4-loopback image application with bounded three-file import、same-process Human review、read-only incomplete diagnostics、bounded background `fsck`<br>
Planned scope: localhost archive list／export／restore, remote publication adapters, and a production application boundary<br>
...
| `synapse-application` initial routes | AIではexact project ACL、Core preflight、exclusive-TTL permit、trusted Executor、実行後reauthを、Humanではsame-instance admitted proposal、またはtrusted control planeのexact durable Proposal／Decision bindingを再検査したregistration、server-fixed candidate、one-shot permitを束縛し、両方をlive profile／FIFO fenceからCore full validationへ接続 | Authenticator実装自体の強度、HTTP/JWT、restartを越えるACL／profile／permit、旧registration／permitの復元、multi-process ordering、OS sandbox／connector／egress、Projection route、organization／quorum／release／modified／partial |
...
| `synapse-artifact` mapper / same-process workflow / generic contract v1 | trusted collectorが渡したbounded regular-file manifestをdeterministicなnested Treeへ写像する。trusted workflowはRef-empty repository一つをbootstrapし、一つのProposal／DecisionをApplication／Core admissionへ通す。公開contractは3 dispositionとsource attributionを固定する | host path収集の安全性、real-user identity、model実行証明、既存project／連続Proposal、journal／recovery統合、restart resume、HTTP／CLI／UI、production deployment、tagged distribution |
| `synapse-artifact-journal` | opaque `ReviewId`とserver-owned Proposal／Decision binding、review state、Decision intentのdomain-separated digestによる同一request replayをseparate SQLiteへ保存する | authentication／authorization、portable permit、Core admission proof、repository inspection、outcome reconciliation、raw idempotency key／rationaleの保存、ObjectStore／RefStoreとのatomic transaction |
...
| `synapse-publication` / `synapse-present` | existing read-only CASと、checkpoint済みRef SQLiteのdigest検証付きprivate stable copyから、private rationale／internal Actor ID／path／raw assetを除外したdeterministic local bundleとchecksumを生成・検証する | 作者性、真実、権利、公開許可の自動判定、OIDの非機密性、remote publication、training-use policyの技術的強制 |
...
flowchart LR
    ...
    APP --> AI
    APP --> HUMAN
    Collector["Trusted artifact collector<br/>bounded regular files"] --> Artifact["synapse-artifact<br/>validate + deterministic Tree mapping"]
    Artifact --> CAS
    Generic["Trusted same-process artifact workflow<br/>one Proposal + one Decision"] --> Artifact
    Generic --> APP
    Binding["Trusted durable Proposal / Decision binding"] --> APP
    Transport["Journal / recovery / transport integration<br/>not implemented"] -. future composition .-> Generic
    Transport -. future composition .-> Journal[("Separate SQLite artifact journal<br/>locator + state + intent digests")]
    ...
    class U,A,CLI,Request,Control,AUTH,Collector,Generic,Transport,Binding,Journal,Proj boundary;
```
...
restart後のlocal review再登録では、trusted control planeが保持する`DurableProposalBinding`を使い、新しく構築した
current application instanceで有効なHuman profile、exact project、profile generation、canonical Decision Ref、live
Proposal Ref/head、live Decision Ref/headを再検査して通常のone-shot registrationを新しく作る。これは旧handle、
registration、permitを復元せず、binding自体もcredentialやbearer authorityではない。公開`ReviewId`は
認可後にserver-owned bindingを探すlocatorにすぎない。separate artifact journalはそのbindingとstate、
idempotent Decision intentを保存するstorage primitiveであり、lookupだけでauthorityを再構成したり
Core admissionを証明したりしない。最終publicationは通常どおり`HumanDecisionRuntime`のfull validation／CASを通る。
bindingは改竄されていないtrusted journalを前提とし、元Proposalが特定processのruntime capability intersectionを
通過したことを暗号学的に証明しない。durable admission evidenceは[#23](https://github.com/howlrs/synapsegit/issues/23)の残課題である。

`synapse-artifact`はtrusted collectorが既にmemoryへ取り込んだregular-file manifestだけを扱う。caller-selected
host pathをopenせず、symlink等のnon-regular entryを拒否し、全manifest validation後にappend-only CASへBlob／Treeを
保存するがmapper単体はRefを更新しない。trusted same-process workflowだけがRef-empty repositoryのbaseを低水準
bootstrapし、その後の一つのProposal／DecisionをApplication／Core admissionへ通す。workflow内credential、identity、
path、profile、candidateはtrusted process configurationであり、OS user認証やbrowser requestではない。
`PendingArtifactProposal`はnon-serializableかつone-shotで、journal／recovery未統合である。Rust receiptも
wire JSON DTOではない。v1 type／workflowはcaller-supplied attributionだけを許可し、executionを常に未検証とする。
verified executor mode、HTTP／CLI／model invocation／restart-resumable／production orchestrationは現時点で未実装である。Creator Pilot／localhost UIは
従来どおりimage-specificで、artifact journalを使ったrestart-resumeを提供しない。
...
500 UTF-8 bytes以内のexact Ref名を受ける。maintenance `fsck`はpathやlimitを受けず、URLとbodyで同じexact
project keyを確認する。transport crateはlocal service facadeだけへ依存し、
...
`fsck`は有限なprocess-local registryへ予約してからdetached blocking jobで実行する。registryは256 entries／
64 active jobsに制限し、満杯時はactive jobをevictせず最古のterminal resultだけを失効させる。browser disconnectは
jobをcancelせず、restart、失効済みID、unknown IDは`operation_state_lost`になる。結果を自動retryせず、
`last_fsck`もprocess restartを越えない。
...
- localhost maintenanceは互換用`Repository::fsck`を呼ばず、server-fixedな`fsck_with_limits`だけを使う。dirty reportも完走した検査として`clean=false`とaggregate countだけを返し、object OID、path、issue detailをHTTPへ出さない。
- creator beginは1 file 64 MiB／3 files 192 MiB、固定graph増分、localhostがprojectごとに保持できる8件すべてのHuman decision増分を予約し、全CAS生成後かつRef公開前にprospective snapshotをbounded fsckする。decisionも公開前に同じadmission／prospective検査を行う。Ref publication自身のhead validatorも`PreparedClosureVerifier`のbounded Tombstone catalogを使い、互換用のunbounded scanへ戻らない。
...
| localhost maintenance fsck Ref roots / CAS objects | 100,000 / 100,000 |
| localhost maintenance fsck inventoried raw object bytes | 1 TiB |
...
| publication CLI discovered creator sessions（complete／incomplete合計） | 100 |
| publication presentation sidecar | 64 KiB |
| publication title / summary / public note / caption | 300 B / 8 KiB / 5 KiB / 1 KiB |
...
- restart後にtrusted control planeが再登録する場合は、`DurableProposalBinding`のproject、Proposal Ref/head、
  Decision Ref/headと、current-instanceの`HumanAuthorityProfileHandle`を`register_recovered_human_decision`へ渡す。
  applicationはprofileのinstance／generation／suspensionとexact projectを検査し、両Refのlive head、profileの
  canonical Decision Refをbindingへ一致させた場合だけ新しい通常registrationを作る。bindingはClone可能な
  configuration valueでありsecret／bearer permitではない。旧handle／registration／permitは復元されず、
  prepare／publishのauthentication、ACL、TTL、FIFO fence、`HumanDecisionRuntime` validationを省略しない。
...
### Artifact review journal

`synapse-artifact-journal`はCoreのRef SQLiteとは別のSQLite databaseに、public-safeなopaque `ReviewId`と
server-owned `ReviewBinding`、bounded `ReviewState`、一つのDecision intentを保存する。`create_or_get_review`は
同じproject／Proposal Ref/headとexact Decision bindingのresponse-loss retryへ同じlocatorを返し、Decision bindingが
変わったreplayはconflictにする。state transitionはcompare-and-setで、同じstateへの再指定はidempotent、
committed／terminal denialはterminal、`outcome_unknown`は外部reconciliation後のterminal transitionだけを許す。

Decision intentではraw idempotency keyとcanonical requestを別domainのSHA-256 digestへ変換し、同じkeyと
同じrequest fingerprintだけをreplayとして受理する。databaseに残すのはdigest、candidate head、feedback OID、
expected Decision headであり、credential、permit、Actor／Policy／Grant OID、repository path、raw key、rationale、
canonical request bytesは保存しない。journal transactionとObjectStore／RefStore transactionはatomicではないため、
`outcome_unknown`を含む結果の照合は上位applicationの責任である。`ReviewId`取得、state、intentの存在を
authentication／authorization／Core admission proofとして扱わない。
...
flowchart LR
    ...
    DB["Trusted DurableProposalBinding<br/>exact live Ref/head recheck"] --> HC
    RID["Authorized ReviewId lookup<br/>locator, not authority"] -. server binding .-> DB
...
admitted proposal handleまたはchecked durable bindingからone-shot Human publicationへ至るnarrow route、各Core full admissionは実装済みである。
...
server-fixed candidateを登録した場合、またはtrusted durable bindingをlive profile／Refsへ一致させて
再登録した場合だけrequest permitを発行できる。handleはdenial後の修正版へ再利用できるが、
registrationとpermitはone-shotである。generic artifact contract v1が公開するdispositionは
`adopted_unchanged`／`rejected`／`deferred`の3つであり、このCore routeの`experiment_only`を公開contractへ含めない。
...
保全され、`creator_session_incomplete`になる。通常のresource limitはprospective検査によりHuman Decision CAS前に失敗する。
localhost serviceはprojectごとのprocess-local writer gateをbegin／decision全体で共有し、同一service内のprospective検査と
Ref publicationを協調的に直列化する。これはcross-process lockではないため、service稼働中に別CLI、別service instance、
direct Repository writerを同じrepositoryへ向けないことを運用前提とする。
defaultのauthorization Clockはprocess-wide stateでwall clockの前進を採用し、巻き戻り中は`Instant`の
経過時間を下限として進める。creatorのrecordingも同じClockを使うため、発行直後のGrant時刻と認可観測が
別のwall-clock sampleへ分裂しない。futureへのclock jump後にwall clockが戻っても有効期限を延長せず、
injected Clockのfailureやpreflight後の後退検出は引き続きfail closedである。
CAS後にconcurrent repository change等でresponse rebuildが失敗した場合もcomplete publicationは残り、localhost serviceは
HTTP 200の`committed` variantでexact durable receiptを返し、消費済みreview slotを解放する。publicationを再試行せず、callerは
receiptとcurrent Refsを使って診断する。create-only Pilotはpartial historyを自動resume／cleanup／上書きしない。
...
source `refs.sqlite3`はcheckpoint済みmain databaseかつ最大512 MiBでなければならない。openerはsource
SQLite connectionやread lockを取得せず、source pathをSQLiteへ渡さない。main fileをcreate-newなprivate
...
このlocal profileは同一OS上の悪意あるprocessからの完全なpath-race isolationを提供しない。source directoryを
同一userが置換できる環境ではexportを実行しない。また、normal dropではprivate copyを削除するが、`SIGKILL`や
process abortではtemporary fileが残り得るため、hostのtemporary-directory cleanupを運用要件とする。

destinationはsource自身とその子directoryを拒否し、parentを既存のreal directoryに限定してsymlink
componentを辿らず、同じparentのstaging directoryへ全fileをwrite／syncしてから対応platformのatomic
no-replaceで公開する。destination自身は不存在を要求し、上書きしない。

M0/M1 profileはdefault-denyで、次を固定する。

- sourceのprivate Human rationale、internal Actor ID、repository path、raw asset bytesを含めない。
- title、summary、caption、display name、public decision noteはboundedなregular UTF-8
  `presentation.toml`だけから受け取り、全て`author_supplied`と表示する。source rationaleの公開許可へ変換しない。
- OID、Ref、head、source fingerprintはtechnical provenanceとして含む。これらもrepository間の相関や既知bytesの
  照合に使われ得るため、`--public`と外部copy前の内容reviewを必要とする。
- target省略時は`synapse`、`github`もlocal layout生成に限る。Git／GitHub API、upload、network operationは0である。
- `story.md`はactive Markdownをescapeし、`index.html`は全textをescapeしてJavaScript、form、external resourceを
  含めない。raw mediaをdecode／embedしない。
- `projection.json`をcanonical semantic sourceとし、manifestと`checksums.json`でfixed inventory、digest、target、
  visibilityをbindする。checksumはsender authentication、公開許可、confidentialityを証明しない。
- machine readabilityは学習許可を意味せず`training_use_policy=prohibited`を宣言するが、外部copy後の利用を
  技術的に阻止または回収できない。

`--public`はlocal bundleのvisibilityを明示選択するだけでpublish commandではない。省略時は
...
| maintenance observation state | operation registryと`last_fsck`はprocess-local。restartまたはterminal result eviction後は復元せず、poll IDは`operation_state_lost`になる |
...
| application state durability | exact project map、ACL、profile、registration、permit、FIFO fenceは一process内だけ。restartで旧handle／registration／permitは失効する。trusted `DurableProposalBinding`からlive profileとexact Ref/headを再検査して新しいHuman registrationを作れるが、ACL／profileのdurable化やmulti-process linearizabilityではない |
...
| Human Decision identity | applicationはinjected Authenticatorとprocess ACLを使い、same-instance admitted proposal、またはtrusted control planeが与えたdurable bindingとlive Ref/headのchecked recoveryを要求する。credential format／MFA／persistent membership DBを実装しない |
| admitted proposal handle | successful AI publicationのsame-process evidenceだけ。portable receipt、signature、restart後のproofではなく、別application instance／projectへ移せない。`DurableProposalBinding`はこのhandleを復元せず、新規registrationの検査入力になるだけ |
| artifact review locator / journal | `ReviewId`は認可後のlookup locatorで、credential、capability、permit、Core admission receiptではない。separate SQLite journalはRef／ObjectStoreとatomicでなく、repository inspection、reconciliation、authority reconstructionを行わない |
| generic artifact workflow | regular-file mapper単体はRefを更新しない。trusted workflowはRef-empty repository一つでProposal／Decision一回だけをApplication／Coreへ通すが、real-user authentication、host collector、既存project／連続Proposal、journal／recovery統合、restart resume、HTTP／CLI／model invocation、generic browser UI、production deployment、tagged distributionは未実装 |
| Human Decision scope | Core application routeは`decision/*`と4 disposition、generic artifact contract v1はadopt unchanged／reject／deferの3 dispositionだけ。organization／quorum、release、`adopted_modified`／`partially_adopted`は未実装 |
...
| creator Pilot partial failure | base Ref公開後かつHuman Decision前はincomplete sessionをlive historyとして残す。Decision publication後はcomplete sessionを残し得る。create-only image実装はartifact journal／recovery registrationへ未接続で、どちらも自動resume／cleanupしない |
...
| publication confidentiality | allowlistでprivate source fieldとraw assetを除くが、公開bundleにはOID／Ref／head／fingerprintとauthor-supplied textが残る。外部copy後のaccess control、削除、training policy enforcementは提供しない |
| publication portability | atomic directory no-replaceを実装するLinux、Android、Apple、Redoxだけがfinal publicationをsupportする。他targetは`storage_error`でfail closedする |
...
- durable／distributed project ACL、profile、registration、permit、multi-process publication fence（trusted bindingからのchecked Human re-registrationとseparate local review journalは実装済みだが、これらを代替しない）
...
- GitHub／Synapse remote publication client、credential、destination diff、remote receipt、retry／idempotency
...
- generic artifactのHTTP／CLI／model invocation／browser editing transportとCreator Pilotへのrestart-resume統合
