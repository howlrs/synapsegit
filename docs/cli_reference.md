...
bounded session timeline, but the CLI has no general projection rebuild or query command.
...
- `original`、`current`、`ai-output`はraw bytesのopaque Blobとしてstreaming ingestする。media type、EXIF、
  pixel、画像内容、差分を解釈しない。original／currentという意味もcaller-suppliedである。
...
timelineのAI Activityはcurrent proposal transitionと一致しなければならない。
...
| `creator_report_invalid` | current proposal／Feedback／decision snapshot／AI Activityのcreator lineageが期待形と一致しない |
