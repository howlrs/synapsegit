...
空directoryは空repositoryとして開かれる。v0.5.1はproject画面からsessionを作成でき、
...
v0.5.1ではoriginal／current／caller-supplied AI outputの三fileをbounded stagingへuploadし、proposalを
...
server-bounded background `fsck`／poll UIもtagged v0.5.1の`synapse-local`に含まれる。
...
### v0.5.1 tagged sourceのgeneric-artifact library

v0.5.1 tagのworkspace sourceには、bounded regular-file mapper、固定generic-artifact v1 contract、
...
v0.5.1 release archiveは`synapse`、`synapse-local`、`synapse-present`の三binaryだけである。このlibraryに対する
