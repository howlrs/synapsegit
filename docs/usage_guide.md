...
空directoryは空repositoryとして開かれる。v0.6.0はproject画面からsessionを作成でき、
...
v0.6.0ではoriginal／current／caller-supplied AI outputの三fileをbounded stagingへuploadし、proposalを
...
### v0.6.0 tagged sourceのgeneric-artifact library

v0.6.0 tagのworkspace sourceには、bounded regular-file mapper、固定generic-artifact v1 contract、
...
v0.6.0 release archiveは`synapse`、`synapse-local`、`synapse-present`の三binaryだけである。このlibraryに対する
