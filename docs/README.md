...
single-user／loopback-onlyのcreator-facing image applicationはarchitectureとversioned HTTP contractまで
決定済みだが、server、route、UIはまだ実装されていない。このapplication sliceはformal Core Stage 1ではなく、
Core v0.1は引き続きStage 0 draftである。
...
flowchart LR
    ...
    RP -. planned .-> UI["localhost image application<br/>architecture / API contract complete"]
    ...
    class A,B,C,D,E,O,CR,P,RP done;
    class J,G,HA,I partial;
    class F,H,K,Q,UI next;
```
...
| single-user localhost image application（safe facade、loopback HTTP、server-rendered UI） | architecture／API contract完了、実装未着手 | [Localhost application architecture](./localhost_application_architecture.md)、[OpenAPI contract](../api/local/v1/openapi.json) |
...
4. [Runtime architecture](./runtime_architecture.md) と [Stage 0 execution plan](./stage0_execution_plan.md)
5. [Core 構想](./core_concept.md) と [使用ガイド](./usage_guide.md)
6. [Project Chrono-Synapse 初期企画](./init_plan.md)
