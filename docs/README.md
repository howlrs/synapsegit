...
tagged v0.8.1 sourceには、bounded regular-file manifestをportable NFC pathとして検証し、Refを更新せず
...
authority、production serviceを提供しないsource-level application building blockである。v0.8.1のrelease
...
このbinaryはv0.8.1 archiveに含まれる。
tagged v0.8.1 sourceにはこれと分離した
...
| v0.8.1の変更と配布境界を確認する | [Release notes](./releases/v0.8.1.md) | [Project status](./project_status.md) |
...
認証、network transport、運用監視まで完成したという意味ではない。tagged v0.8.1 source／workspace
...
v0.8.1のtagged UIも引き続き三画像専用で、保持中のpending reviewをrestart後に再開しない。generic boundaryにも
