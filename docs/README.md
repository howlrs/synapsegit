...
current `main`はarchive export APIまで実装済みである。export UIとarchive restore API／UIは未実装である。
...
| single-user localhost image application（safe facade、loopback HTTP、server-rendered UI） | slices 1-4/6、slice 7のbounded fsck／job基盤、slice 8のread-only diagnostics部分を実装済み。bounded read-only archive listing（`--archive-root`起動flag指定時のみ有効）はtagged v0.6.0 binaryに含まれ、current `main`はexact confirmation付きbounded no-replace archive export APIも実装する。export UIとrestore API／UIは未実装 | [Localhost runbook](../deploy/local/README.md)、[Localhost application architecture](./localhost_application_architecture.md)、[OpenAPI contract](../api/local/v1/openapi.json) |
