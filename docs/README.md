...
authority、production serviceを提供しないsource-level application building blockである。
...
| generic regular-file mapper／sequential Proposal workflow／frozen public-safe v1 contract | library／application contractを実装済み。fresh attempt、one-active-review、accepted manifest再検証、host approvalを含む。HTTP／CLI／UIは未実装 | `synapse-artifact`、[Generic artifact v1](../spec/application/generic-artifact/v1/README.md) |
...
| generic-artifact bounded checkout／local public projection | one Ref snapshot、exact Decision lineage／protected controls／selected site／digestをbounded検証し、versioned canonical JSON／Markdown／script-free HTML／local Synapse・GitHub layoutを生成。remote publishではない | `synapse-artifact`、`synapse-publication`、[profile](../spec/application/generic-artifact-publication/v1/README.md)、[roadmap](./generic_artifact_publication_roadmap.md) |
