...
tagged v0.5.1 sourceには、bounded regular-file manifestをportable NFC pathとして検証し、Refを更新せず
...
authority、production serviceを提供しないsource-level application building blockである。v0.5.1のrelease
...
このbinaryはv0.5.1 archiveに含まれる。
tagged v0.5.1 sourceにはこれと分離した
...
`--archive-root`起動flag）として実装済み。archive export／restoreのAPI／UIは未実装である。
...
| v0.5.1の変更と配布境界を確認する | [Release notes](./releases/v0.5.1.md) | [Project status](./project_status.md) |
...
| single-user localhost image application（safe facade、loopback HTTP、server-rendered UI） | slices 1-4/6、slice 7のbounded fsck／job基盤とbounded read-only archive listing、slice 8のread-only diagnostics部分を実装済み。archive export／restoreのAPI／UIは未実装 | [Localhost runbook](../deploy/local/README.md)、[Localhost application architecture](./localhost_application_architecture.md)、[OpenAPI contract](../api/local/v1/openapi.json) |
...
認証、network transport、運用監視まで完成したという意味ではない。tagged v0.5.1 source／workspace
...
v0.5.1のtagged UIも引き続き三画像専用で、保持中のpending reviewをrestart後に再開しない。generic boundaryにも
