...
current `main`には、bounded regular-file manifestをportable NFC pathとして検証し、Refを更新せず
...
同crateのtrusted same-process workflowはRef-empty repository一つをbootstrapし、Application／Coreを通した
Proposal一つとadopt／reject／defer Decision一つを記録する。
server-owned Proposal／Decision bindingは別SQLiteの`synapse-artifact-journal`へopaque `ReviewId`と
bounded state／idempotent Decision intentとして保持できる。restart後はpermitを復元せず、trusted
`DurableProposalBinding`からcurrent Refを確認してordinary one-shot Human registrationを作り直し、
final publicationを引き続き`HumanDecisionRuntime`へ渡す。これらはHTTP／CLI／browser UI、model invocation、
multi-process authority、production serviceを提供しないsource-level application building blockである。
same-process workflowはjournal／recovery未統合で、restart resume、既存repository、連続Proposalを扱わない。
...
flowchart LR
    ...
    AR["Generic artifact boundary<br/>regular files / frozen v1 contract"] --> GW["Same-process generic workflow<br/>one Proposal + one Decision"]
    GW --> C
    GW --> HA
    AJ["Separate review journal<br/>opaque locator / idempotent intent"] -. remaining integration .-> GT["Restart / transport orchestration<br/>not implemented"]
    GW -. remaining integration .-> GT
    ...
    class A,B,C,D,E,O,CR,SM,P,RP,UI,PB,AR,AJ done;
    class J,G,HA,I,GW partial;
    class F,H,K,Q,CL,GT next;
```
...
| generic regular-file mapper／same-process one-Proposal workflow／frozen public-safe v1 contract | library／application contractを実装済み。workflowはRef-empty repository一つに限定し、journal／recovery統合、restart resume、HTTP／CLI／UIは未実装 | `synapse-artifact`、[Generic artifact v1](../spec/application/generic-artifact/v1/README.md) |
...
| separate SQLite artifact review journal | opaque `ReviewId`、server-owned binding、bounded state、hashed／fingerprinted Decision intentを永続化。authority／Core receiptではない | `synapse-artifact-journal` |
...
認証、network transport、運用監視まで完成したという意味ではない。current `main`のsource／contract
boundaryと明記したrowはuntagged source／schemaの存在を示し、transport contract test完了を意味しない。
generic artifact v1はCore v0.1とは別のapplication contractである。`ReviewId`は認証済みproject lookup用の
locatorにすぎず、Ref／head／OID／permitをpublic DTOへ露出しない。journal rowはauthorityではないため、
embedding serviceはlookup前にauthentication／project authorizationを行い、journalのserver-owned bindingから
`DurableProposalBinding`を再構築する。recovery registrationはcurrent Proposal Ref/headとcanonical Decision
Ref/head、live Human profileを一processのFIFO fence内で照合するだけで、古いhandle／permitをdeserializeしない。
その後のprepare／publishはnormal one-shot routeを使い、final Decisionはfull Core validationとCASを通る。
exact project map、ACL、profile、permit、fence自体は引き続きprocess-localで、multi-process durabilityではない。

`begin_artifact_proposal`／`decide_artifact_proposal`はtrusted config、accepted／proposed manifest、
canonicalizable application contextから一つのsame-process flowを構築し、AI ProposalとHuman Decisionを
Application／Core admissionへ渡す。返すpending authorityはnon-serializableかつone-shotである。Rust receiptは
...
このgeneric source-level boundaryをv0.3.0 Creator Pilot／localhost UIのresume機能と混同しない。tagged UIは
引き続き三画像専用で、保持中のpending reviewをrestart後に再開しない。generic boundaryにもHTTP／CLI／model
provider実行、public publication、production deployment、配布許可は含まれない。
