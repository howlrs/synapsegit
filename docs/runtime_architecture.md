...
`begin_artifact_proposal`はtrusted `TrustedArtifactProjectConfig`、accepted／proposed manifest、
canonicalizable application context、source attributionを受け、Refが一つもないrepositoryを一回だけbootstrapする。
base Decision Refはtrusted bootstrapで作り、Proposalは`Application`／`CreativeAiRuntime`のauthenticated
one-shot routeを通す。返す`PendingArtifactProposal`はnon-serializableなsame-process Human authorityである。
`decide_artifact_proposal`はそのpending valueを一度だけ消費し、`adopted_unchanged`／`rejected`／`deferred`を
通常のApplication／`HumanDecisionRuntime` routeでDecision Refへpublishする。既存Refを持つrepository、
同じrepositoryでの連続Proposal、restart resume、journal／recovery連携はこのworkflowに含まれない。
...
`synapse-artifact-journal`はこのbindingを別SQLite databaseへ保存するtransport-neutral storage primitiveである。
`ReviewState`は`pending_review`、`decision_committed`、`terminal_denial`、`retryable_failure`、
`outcome_unknown`に限定する。exact bindingの`create_or_get_review`は同じopaque locatorをreplayし、同じProposal
identityへ異なるDecision bindingを与えると`review_binding_conflict`になる。Decision intentはraw idempotency keyと
canonical requestをdomain-separated SHA-256でmemory内hashし、digest／fingerprintとserver candidate／expected
bindingだけを保存する。同じkey／exact fingerprintはreplayし、変更は`idempotency_conflict`である。raw key、
rationale、canonical request、credential、permit、Actor／Policy／Grant OID、repository pathはschemaに入れない。
state CASはsame-stateをidempotentに扱い、committed／terminal denialをterminalにし、`outcome_unknown`からは
committed／terminal denialへのreconciliationだけを許す。journal自身はauthentication、repository inspection、
Core receipt、reconciliation proof、authority reconstructionではない。
binding／journalはtrusted local storageを前提とし、元Proposalが特定processのruntime capability intersectionを
通過したことを暗号学的に証明しない。durable admission evidenceは[#23](https://github.com/howlrs/synapsegit/issues/23)で追跡する。

このC1 boundaryはsource-level Rust libraryであり、HTTP／CLI、generic browser UI、model provider invocation、
multi-process linearizability、public publication、production deployment、tagged binary、新たな配布許可を提供しない。v0.3.0 Creator Pilotと
localhost UIは画像専用のままで、同じprocessに保持したpending authorityをrestart後にresumeしない。
same-process generic workflowもjournal／recoveryを自動利用せず、restart後にpending authorityを再開しない。
...
Proposal／Decision commitを証明しない。未実装のdurable／transport orchestrationがsame-process workflow、
recovery、journalを組み合わせる場合は、
Proposal publication成功後にserver bindingを`create_or_get_review`し、Decision前にexact candidate／request intentを
idempotently固定する。timeout／crash後はblind replayせず、journal bindingをtrusted project configurationとlive
Core stateへ照合してからrecovery registrationまたはbounded terminal／unknown stateへ進める。journal transactionと
Core CASを一つのatomic commitと表現してはならない。
