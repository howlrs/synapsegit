...
Applies to: tagged v0.5.0 source/workspace libraries

Decision date: 2026-07-11

## 結論

Core実装言語は**Rust**とする。local repository pathに加え、ordered Observationのdeterministic byte-identity baselineを`synapse-observation`、process-localなauthenticated one-shot AI executionとnarrow Human Decision routeを`synapse-application`、Creative AIのpreflight／proposal publication admissionを`synapse-core::CreativeAiRuntime`、narrow Human Decision admissionを`HumanDecisionRuntime`としてRustに置く。generic regular-file application向けには`synapse-artifact`のbounded mapper／checkout、固定v1 contract、sequential Proposal／Decision、host-authenticated approval、別SQLite journalとexplicit restart reconciliationを置く。ただし正本をSurrealDBへ閉じ込めず、Gitライクな不変objectをfilesystem/object storage上のCASに置く。ローカルのRef・reflogはSQLite、public共有serviceのRef／reflog authorityはPostgreSQLを採用するproduction targetとした。query用には`synapse-projection::SqliteProjectionStore` baselineを実装済みである。これはverified ObjectStoreとcaller-suppliedな一貫したRef snapshotから破棄・再構築する派生indexで、SurrealDB adapterと全8 query／benchmark比較は未実装である。作者外への表示は別の`synapse-publication`がcreator Ref SQLiteのstable private copyまたはgeneric Decisionのbounded checkoutからprovider-neutralなlocal PublicProjection／bundleへ変換する。
...
- bounded regular-file manifestのportable path validationとdeterministic nested ManifestTree mapping
- fixed generic-artifact v1 DTO、trusted durable Proposal bindingのchecked recovery registration、
  separate SQLite review／idempotency journal
- SQLiteのgeneric cross-Ref preconditionによるatomic `stale_base`判定
- current Ref closureだけを索引するdisposable SQLite ProjectionStore
- ordered Observationと全media dependencyを検証し、primary Blob OIDだけを比べるdeterministic byte-identity adapter
...
### Rust-rendered HTML: first Creator-facing application

- Axumのloopback-only transportとAskamaのserver-rendered semantic HTML
- project／Ref／history／report、3-file import、proposal review、maintenance UI
- small browser-native ES moduleによるfile preview、fetch submission、job polling
- transport-neutral local service facadeが作るversioned DTO

HTTP crateはlocal service facadeだけへ依存し、Core／SQLiteを直接importしない。これによりtrusted operator向け
`update-ref`をroute化しにくい依存境界を作る。public serviceではなく、exact Host／Origin、process-local browser token、
no-CORS、CSP、bounded uploadを持つsingle-user localhost applicationである。詳細とmachine-readable contractは
[Localhost application architecture](./localhost_application_architecture.md)を参照する。

### TypeScript: future rich client / SDK option

- canvasを使うpixel／mask／registration viewer
- client-side stateが必要なdesktop/web UI
...
## Deterministic Observation byte-identity baseline

`synapse-observation`の最初のadapterは`base_observation`→`target_observation`をordered inputとして扱う。
schema-validな両Observation、参照CaptureProfile、primary／preview等の全media Blobをverified CASから読み、
各Observationで一意な`role=primary` Blob OIDが同じかだけを比較する。media bytesのdecode、EXIF検査、
viewpoint registration、pixel差分は行わない。したがって同じbytesは物理対象の無変更を、異なるbytesは
物理／visual changeを証明しない。比較成功時もAnalysisResultは`comparability=partial`、reason
`byte_identity_only`である。subject／series mismatchや`role=primary` entryが0件／複数件の場合はnormal resultの
`not_run`／`incomparable`とし、missing／corruptなObservation、CaptureProfile、mediaはwrite前にhard errorとする。

AnalysisResultはadapter id/version、ordered input、determinism、implementation／configuration OIDを保持する。
implementation Blobはsemantic Rust sourcesとcrate manifestをlength-framedしたdeterministic bundle、configuration
Blobは固定JSONである。ただしimplementation digestは`Cargo.lock`、transitive dependency source、compiler、target、
OS／runtimeを束縛しないため、Projectionの`Ready`と同様にexact replay保証ではない。adapterはこれらimmutable
...
## Generic artifact application boundary

`synapse-artifact`は、trusted serverが選んだin-memory manifestをgenericなregular-file artifactへ
写像する上位Rust boundaryである。directoryはrelative NFC pathに暗黙化し、regular file以外、absolute／
traversal／backslash／control path、duplicate／case／file-directory collision、file count、per-file／aggregate
bytes、path bytes、depth超過をmanifest construction時に拒否する。全manifest validationは最初のCAS writeより
前に完了する。mapperはraw file bytesをBlobへ、directoryをnested ManifestTreeへ決定的に変換するが、Ref、
reflog、Commit、Proposal、Human Decisionを単独では作らない。同じvalidated path／bytesはrepository locationや
input orderに依存せず同じsite Tree OIDになる。

`prepare_artifact_proposal`／`publish_prepared_artifact_proposal`はtrusted
`TrustedArtifactProjectConfig`、accepted／proposed manifest、canonicalizable application context、source
attributionからimmutable graphを先に準備し、その後base Decision RefとProposal Refを公開する。Proposalは
`Application`／`CreativeAiRuntime`のauthenticated one-shot routeを通す。`prepare_next_artifact_proposal`は
canonical Decision Refのexact current headからdeterministic attempt identity／fresh Proposal Refを導出し、current
Decisionでselectedされたsiteとaccepted manifestのbyte/path identityが一致する場合だけ次Proposalを許可する。
同じbase headでは一つのattempt Refしかなく、concurrent publicationはcreate-only CASで一件だけ成功する。Decision後も
旧Proposal Refを保持し、new Decision headから次attemptを導出する。

返す`PendingArtifactProposal`はnon-serializableなsame-process Human authorityである。
`ArtifactApprovalRegistry`はembedding hostの`Authenticator`とserver-owned project ACLを使い、actor/session、ACL
epoch、exact Proposal／expected Decision binding、disposition、rationale presence／bytes、expiryに束縛したopaque
`ArtifactDecisionApproval`を発行する。claimはauth／ACL後、Decision object／Ref mutation前にone-shotでburnする。
`prepare_artifact_decision`／`publish_prepared_artifact_decision`はfresh live-binding preflightを行い、その後通常の
Application／`HumanDecisionRuntime` routeで`adopted_unchanged`／`rejected`／`deferred`をpublishする。
v1 typeとconvenience workflowは`caller_supplied_ai_attributed`だけを受け、executionを常に未検証とする。
verified executor modeはv1に存在せず、model providerも呼ばない。

公開application contractは
[`spec/application/generic-artifact/v1`](../spec/application/generic-artifact/v1/README.md)に固定する。
全payloadは`contract=synapsegit.generic-artifact`とinteger version `1`を持ち、unknown field／version／enumを
fail closedにする。MVP dispositionは`adopted_unchanged`、`rejected`、`deferred`だけである。
`caller_supplied_ai_attributed`は常にexecution未検証であり、request fieldだけでverified executionへ昇格しない。
public proposal／review DTOはopaque `ReviewId`とapplication-level manifest／redacted-context digestだけを持ち、
repository path、Ref／head、Core OID、Actor／Policy／Grant、permit、credentialを出さない。`ReviewId`は
authentication後のlookup locatorであり、bearer authorityではない。
これはfrozen JSON transport contractのDTOである。一方、現Rust workflowの`ArtifactProposalReceipt`／
`ArtifactDecisionReceipt`はmanifest／context digest、attribution、disposition、selected snapshotをgetterで返す
non-Serde process valueで、`review_id`／stateを含むJSON schemaのwire implementationではない。

restart recoveryのapplication primitiveは`DurableProposalBinding`と
`Application::register_recovered_human_decision`である。bindingはserver-owned project、Proposal Ref/head、
canonical Decision Ref/headをtrusted control planeが再構築するnon-Serde valueで、`Debug`もredactする。
recovery methodはcurrent application instanceのHuman profile handle、exact project、unsuspended/current profile
generation、profile Decision Ref、live Proposal Ref/head、live Decision Ref/headをexisting process-local FairGate内で
照合し、通常と同じone-shot `RegisteredHumanDecisionHandle`を作る。古いAdmittedProposalHandle、registration、
permitを復元せず、自身ではauthenticationもしない。その後に通常の`prepare_human_decision`／
`publish_human_decision`が必要で、final immutable validation、duplicate check、proposal precondition、Decision CASは
必ず`HumanDecisionRuntime`を通る。FairGate、ACL、profile、permitは引き続き一process内だけである。

`synapse-artifact-journal`はbindingを別SQLite databaseへ保存するtransport-neutral storage primitiveである。
schema v2はprivate Proposal intentをProposal CAS前に登録し、verified publication後だけpublic-safe `ReviewId`を
finalizeする。Decisionはraw idempotency key／canonical requestをdomain-separated digestへ変換し、stored binding、
disposition、selected snapshot、reviewed manifest digest、expected/new Decision head、feedback OIDをCAS前intentへ
固定する。exact Core receiptを検査した後だけverified outcomeと`decision_committed`を一transactionで確定する。
同じexact requestはprivate intent／ReviewId／outcomeをidempotently replayし、異なるbinding／fingerprintはconflictに
する。credential、approval、permit、Actor／Policy／Grant OID、repository path、raw key、rationale、canonical request
bytesはschemaに入れない。legacy rowをverified v2 outcomeへ暗黙昇格しない。

`synapse-artifact`のexplicit durable orchestrationはProposal prepare後・CAS前にjournal intentを書き、ordinary
Application/Core publication後にbindingをfinalizeする。Decisionではhost approvalをclaimしてordinary Human permitを
prepareし、exact intentをjournalへcommitしてからCore CASを行い、receiptとbounded checkoutが一致した後だけoutcomeを
commitする。restart／timeout後はtrusted project configでauthentication／ACLをlookup前に行い、journal bindingを
immutable Proposal graph、一つのlive Ref snapshot、reflogへ照合する。CAS前ならexact requestからprepared Proposalを
再生成し、Proposal CAS済みならgraphからfresh Application authorityを導出する。Decision CAS済みならexact reflogと
selected-site checkoutからoutcomeを確定し、CAS未実行を証明できたintentだけをretryableへ戻す。old credential、
`AdmittedProposalHandle`、approval、registration、permitは復元しない。

journal transactionとObjectStore／RefStore transactionはatomicでないため、このreconciliationを省略してjournal rowを
Core proofとして扱わない。binding／journal／trusted configは改竄されていないlocal storageを前提とし、元Proposalが
...
このC1 boundaryはtagged v0.5.0 sourceに含まれるworkspace Rust libraryである。release archiveは
...
Policy／Grant、Human／AI Actor、Subject、bootstrap control profileとprotected `base` chainをexactに検証し、authority
探索でselected／non-selected siteを走査しない。dispositionが選んだdirect `site` Treeだけをnode／edge／file／path／
per-file／aggregate byte上限内で読み、Blobは上限を超えてmaterializeせずOIDを検証する。全regular-file manifestと
trusted digestが一致するまでpartial resultを返さない。

`synapse-publication`のgeneric-artifact v1 profileはこのcheckout結果からだけcomplete public projectionを作る。
pending／incompleteはnon-serializable trusted display statusから作り、repository locator、Ref／head／OID、digest、
Decision receipt、selected siteを含めない。strict `ReviewedPublicTargetV1` allowlist、canonical JSON、escaped Markdown、
script-free HTML、fixed manifest／checksum、local Synapse／GitHub layoutをcreator publication profileと分離してversioned
dispatchする。generator labelは安全なASCII versionでmanifest／projection一致を要求するが、crate release versionと
固定せず、同じrenderer profile v1の過去bundleを再検証できる。Git／network operation、remote adapter、Git import／
provenance、identity mapping、GitHub App、hosted serviceは含まれない。
...
区別する。Ref head自身を除きtombstonedだけならclosureはcompleteになり得る。一方、corrupt byte、
...
original／current Observationは同じimported／reference-only CaptureProfileを参照する。CreatorはAI agentとは
別EntityIdの`software_tool` Actorを作り、original→currentのbyte-identity AnalysisResult、implementation／
configuration Blobとともにbase Treeへ置く。base Commitのtransitionはimport ActivityとAnalysisResultを参照するため、
その比較一式は後続のproposal／decision両Refから到達する。imported profileは撮影条件がrepeatableまたはcalibrated
であるというclaimではない。
...
base Tree内のSubject extension `org.synapsegit.creator-session`がsession名とcore session IDを保持する。
comparison tool／analysis IDは保存されたseries IDから決定的に再導出する。`creator-report`はdecision headから
base Commit／Tree／Subjectへ辿ってmanifestを読み、archive restore後にも同じIDを復元する。
...
`selected_ai_output`を返す。加えてProjection `analysis_lineage`からordered Observation input、adapter／config、
dedicated tool Actor、availability-only replay readiness、proposal／decision両Ref reachabilityを検証してbyte identityを
報告する。archive restore後にも同じOIDとreportを再構築する。比較一式を持たないlegacy-shaped creator snapshotは
`comparison=unavailable`として読めるが、そのshapeは作成時期を証明しない。一式の一部だけがあるsnapshotは
invalidとして拒否する。adoptでは
decision snapshotがproposal snapshotに一致してselectedがtrue、reject／deferではbase snapshotに一致してfalseになる。
...
artifact review journalのSQLite transactionはCore Ref/reflog transactionと別であり、journal stateだけで
Proposal／Decision commitを証明しない。generic artifact durable orchestrationはProposal／DecisionそれぞれのCAS前に
exact private intentをidempotently固定し、ordinary Core publication後にreceipt、live Ref／reflog、immutable graph、
selected-site checkoutを段階に応じて検証してjournal publication／outcomeを確定する。timeout／crash後はblind replayせず、
lookup前のfresh auth／ACLとtrusted project configからreconcileする。これはlocal explicit recovery boundaryであり、
journal transactionとCore CASを一つのatomic commitと表現してはならない。
...
crates/
  ...
  synapse-observation deterministic ordered Observation byte-identity baseline
  synapse-application local authenticated Creative AI + narrow Human Decision routes
  synapse-artifact    bounded regular-file mapper / frozen generic-artifact v1 boundary
  synapse-artifact-journal separate SQLite review binding / state / idempotent intent
...
`synapse-artifact-journal`もRefStoreやProjectionStoreとは別のapplication databaseであり、
そのrowはauthorization、Core receipt、accepted historyの正本ではない。
...
`DurableProposalBinding` recovery registrationとlocal artifact journalはold handle／permitをportableにせず、
generic durable orchestrationもfresh process authorityでlocal crash windowをreconcileするだけで、cross-process
FairGateやatomic Core+journal transactionを提供しない。従ってprovider adapter追加だけでは
production化できず、streaming object port、PostgreSQL unit of work、
durable operation／admission contractへ分離する必要がある。

最初の実装単位`put-object → verify OID → build tree → commit → CAS ref → fsck → export/restore`、`ordered Observation → deterministic primary Blob OID comparison → AnalysisResult`、`authenticate → exact project ACL → Core preflight → one-shot execution → reauth/fence → full proposal publication → admitted handle`、`authenticate human → registered admitted proposal/candidate → one-shot permit → full supported disposition/CAS`、`bounded regular files → deterministic nested site Tree → sequential fresh Proposal／Decision`、`host auth + project ACL → exact opaque approval → burn before Decision mutation`、`pre-CAS journal intent → ordinary Core CAS → live reconcile → bounded checkout → verified outcome`、`consistent RefSnapshot + CAS → atomic SQLite projection rebuild`、`3 opaque files → CaptureProfile／byte Analysis／Creator provenance → AI/Human publication → snapshot-bound report`、`stable private Ref SQLite copy → bounded RefSnapshot → PublicProjection → deterministic local Human/Machine bundle`、`generic Decision checkout → detached versioned local public bundle`はlocal Rust縦切りとして実装済みである。ただしbyte comparisonはphysical／visual change analysisではなく、application routeはAIとnarrow Human Decisionだけのprocess-local libraryで、Creator Pilotとgeneric artifact workflowはその上に置くtrusted local orchestrationである。generic artifact durable boundaryはlocal trusted config／journalを前提とし、productionのdistributed admission authorityではない。publication targetはlocal rendererだけでremote publishではない。SurrealDB導入はこの正本経路から分離し、optional projection adapterとして並行検証する。
