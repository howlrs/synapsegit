...
> **Document type: informative concept.** この文書はCoreの設計理由と将来ontologyを含む。現在v0.1で符号化されたobjectは[Coreデータモデル](./core_model.md)、実装状態は[documentation index](./README.md)、規範は[`spec/core/v0.1`](../spec/core/v0.1/README.md)を参照する。
...
下表は将来ontologyを含む概念一覧であり、すべてが現在のv0.1 schemaに一対一で存在するわけではない。

- **v0.1で直接符号化済み:** Blob、ManifestTree、Commit、およびActor、Subject、Activity、Observation、Claim、ClaimReaction、CaptureProfile、AnalysisResult、ContextPack、DelegationGrant、DecisionFeedback、Policy、Assurance、EvidenceGap、TombstoneのRecord。
- **概念または将来extension:** Project、Artifact / ArtifactVersion、ProcedureVersion、Session、ObservationSeries、Capture、CaptureProtocol、CaptureStation、StationDeployment、Calibration、SpatialFrame / SpatialTransform等。現在は既存Recordのpayload / extension / relationで試行するか、将来schemaで固定する。
...
ただし現在のnarrow `HumanDecisionRuntime` profileは、Policy／Actor／Grantをsnapshot control比較外から
bindしてHuman Gateを迂回しないよう、candidate Decision Commitの`bound_declaration_refs`をemptyに限定する。
DecisionFeedback `source_refs`はEvidence citationとして利用できるが、effective authorityやbound declarationにはならない。
...
```mermaid
flowchart TB
    Client[Capture Client / Tool Connector / CLI] --> API[Ingest and Record API]
    AIRequest["AI credential + project + opaque handle"] --> AIApp["Local AI application route<br/>preflight / one-shot permit / reauth"]
    AIApp --> AIAdmission["Core AI proposal admission"]
    AIAdmission --> Receipt["Admitted proposal handle"]
    Receipt --> HumanApp["Local Human application route<br/>profile / candidate / one-shot permit"]
    HumanApp --> HumanAdmission["Core Human Decision admission"]
    AIAdmission --> Objects
    AIAdmission --> Refs
    HumanAdmission --> Objects
    HumanAdmission --> Refs
    API --> Objects[(Content-addressed ObjectStore)]
    API --> Refs[(RefStore + reflog)]
    Objects --> Jobs[Analysis Job Queue]
    Jobs --> Adapters[Versioned Adapters]
    Objects --> UX[Timeline / Compare / Review / Export]
    Refs --> UX
...
[SynapseGit Core使用ガイド](./usage_guide.md)を参照する。
...
9. 竣工時に確認範囲を明記したAs-recorded Releaseを作る。追加の検査・契約要件を満たす場合だけ限定付きAs-built表現を使う。
...
この節はconcept作成時のphase groupingを残す。local CAS、CLI、Ref CAS、Tombstone、closure、archiveは計画より早くStage 0で実装済みである。現在の進捗とexit gateは[Stage 0 execution plan](./stage0_execution_plan.md#progress-summary)を正とする。
...
現在library実装済みのHuman Gateは、authenticated single responsible principalによる`decision/*`の
`adopted_unchanged`／`rejected`／`deferred`／`experiment_only`だけである。release、modified／partial、
organization／quorum、その他のgateは上の設計原則に含まれるが未実装である。

Creative AIについては、requestをcredential／project selector／opaque handleまたはpermitへ限定する
process-local application routeを実装済みである。candidate-independent Core preflightからexclusive-TTLの
one-shot permitを作り、permitをburnしてからsingle trusted Executorを起動し、実行後reauth→project FIFO
publication fence→live ACL/profile再構築→full Core revalidation／CASの順で処理する。これはHTTP／JWT、
durable／distributed ACL・permit、multi-process fence、OS sandbox／egressを実装したという意味ではない。

対応するnarrow Human application routeも実装済みである。AI成功receiptの
`AdmittedProposalHandle`をborrowし、reusable Human profileとserver-fixed Decision Commit／Feedback／messageを
one-time registrationへ束縛する。Human requestはcredential、exact project、opaque registration／permitだけを
渡し、application TTL、live ACL／profile、同じFIFO fenceから`HumanDecisionRuntime`のfull validation／CASへ
進む。handleはdenial後の修正版registrationへ再利用できるが、registrationとpermitはone-shotである。
concrete human credential／persistent membership、organization／quorum、release／modified／partial workflow、
Projection routeはこの境界に含まれない。
...
- exact project route、candidate-independent preflight、one-shot execution permit、post-execution reauthorization
- admitted proposal handle、server-fixed Human candidate、one-shot Human registration／permit
- AI runを含むOID golden fixture

最小縦切りは、固定CommitからContextPack生成、authenticated project route、Core preflight、one-shot trusted execution、AI Proposal Branch、DiffとClaim、人による採否、DecisionFeedback、次回Contextへの反映、archive verifyである。

---

## 22. 実装言語と保存境界

Coreの第一候補はRustとする。Gitライクな正本はfilesystem/object storage上のCASに置き、ローカルRef・reflog・既定indexはSQLiteで開始する。initial authenticated AI routeとsame-instance admitted proposalに限定したnarrow Human routeもRustの`synapse-application`としてprocess-localに実装し、authority／target／executor／candidateをuntrusted requestから分離する。SQLite ProjectionStore baselineは現在実装済みで、current Ref closureだけを対象にSubject timeline、Observation dependency、typed AnalysisResult lineage／replay readiness、closure diagnosticsを提供する。SurrealDBはActor–Activity–Observation–Claim–ContextPackの横断探索に有力だが、OIDやarchiveの正本にはせず、同じCASから再構築するoptional adapterと残りの代表query／benchmarkを今後比較する。
