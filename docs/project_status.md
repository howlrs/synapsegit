...
- raw asset／safe derived thumbnail publicationと実利用者による外部理解評価
...
| `synapse-present` binary | Current source only; prepared for the next tagged archive, not included in v0.2.0 |
...
含まれない。`synapse-present`もcurrent sourceで追加されたため、公開済みv0.2.0 archiveには含まれない。

## 次の優先順位

1. synthetic adopt／reject／defer publication bundleでHuman／AI理解、accessibility、link、redactionを検証する。
