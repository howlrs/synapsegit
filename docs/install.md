...
The tagged v0.6.0 source also contains the frozen generic-artifact v1
...
Download the archive and checksum from the fixed v0.6.0 release URL:
...
itself. Verify the v0.6.0 archive's build provenance with GitHub CLI as well:
...
less synapsegit-v0.6.0-x86_64-unknown-linux-gnu/README.md
...
the host. Install directly from the immutable v0.6.0 tag:
...
```bash
git clone --branch v0.6.0 --depth 1 https://github.com/howlrs/synapsegit.git
cd synapsegit
...
- [Read the v0.6.0 release notes](./releases/v0.6.0.md)
