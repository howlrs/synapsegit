...
processが表示した正確な`http://127.0.0.1:...`を開きます。上記でinstallしたv0.1.0のUIは
read-onlyで、作成、review、export、restoreはCLIで行います。current `main`のsource buildでは、
boundedな三file importとsame-process Human reviewもbrowserから実行できます。
...
tagged v0.1.0の`synapse-local` binaryはbrowser import／reviewより前のread-only版です。
write sliceを試す場合はcurrent `main`からsource buildしてください。
