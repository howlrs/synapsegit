...
_実装済みの `synapse-local` read-only画面です。`127.0.0.1` だけで配信され、
hosted serviceやmulti-user serviceではありません。_

## 現在このpreviewを活用できる人

v0.1.0 preview の主な対象は次の利用者です。
...
しています。ただしcapture tool、pixel-level比較、write可能なcreator UI、production
cloud serviceは未実装です。
...
processが表示した正確な`http://127.0.0.1:...`を開きます。現在のUIはread-onlyです。
作成、review、export、restoreはCLIで行います。更新・source build・uninstallは
[install guide](./docs/install.md)、Core全体の往復demoは
...
| local browser UI | project、session、evidence、画像のread-only表示を実装済み |
...
| loopback-only viewerを起動する | [Local application runbook](./deploy/local/README.md) |
...
- [`v0.1.0`](https://github.com/howlrs/synapsegit/releases/tag/v0.1.0)はprereleaseであり、
...
- Release assetにはSHA-256 checksumがあります。更新後のworkflowで作る今後のtagged buildには、
  GitHub build-provenance attestationも付与します。
- object、archive、OID formatはdraftで、stable releaseまでに変わる可能性があります。

評価前に[changelog](./CHANGELOG.md)と
[v0.1.0 release notes](./docs/releases/v0.1.0.md)を確認してください。
