...
The current v0.6.0 preview is most useful to:
...
API and the confirmed bounded empty-target archive restore API; the tagged
v0.6.0 binary remains CLI-only for export and restore, and neither operation
has a localhost browser control.
...
| Capability | v0.6.0 release status |
|---|---|
...
| Local browser interface | Read views, bounded three-file import, same-process `adopt` / `reject` / `defer`, read-only incomplete-session diagnostics, and confirmed background `fsck`. Tagged v0.6.0 adds a bounded, read-only archive listing view (`GET /archives`) behind an optional `--archive-root` startup flag. Current `main` also implements confirmed bounded archive export and empty-target restore APIs; their browser controls remain unavailable |
...
| Read-only history presentation for people and AI | Included in v0.6.0 as a deterministic local bundle: canonical JSON, Markdown, no-JavaScript HTML, manifest, checksums, and Synapse/GitHub target layouts; no upload or network access |
...
row describes library and schema surfaces included in the tagged v0.6.0
...
The tagged v0.6.0 source/workspace libraries include evaluation-only building
...
These capabilities are included in the tagged v0.6.0 source/workspace
...
distribution permission. The packaged v0.6.0 Creator Pilot and localhost UI
remain image-specific; their pending review authority is still same-process
and non-resumable.

The tagged v0.6.0 `synapse-local` binary includes browser import/review,
...
The v0.6.0 archive also provides the separate `synapse-present` companion. It reads
...
Separately, the tagged v0.6.0 source/workspace libraries include a versioned
...
- [`v0.6.0`](https://github.com/howlrs/synapsegit/releases/tag/v0.6.0) is a
...
- Release assets have SHA-256 checksums. The v0.6.0 archive also receives a
