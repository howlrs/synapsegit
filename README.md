...
The current v0.7.0 preview is most useful to:
...
Open the exact `http://127.0.0.1:...` URL printed by the process. The v0.7.0
...
no browser control for either operation. Current main additionally provides
...
Current main also adds a read-only image comparison dialog to pending and
...
Current main also explains each review choice before confirmation, counts
...
| Decision review (current main) | Described adopt/reject/defer outcomes and confirmation, live rationale byte counts, fixed inputs during submission, and escaped recorded-rationale display on completed sessions. No same-session decision changes or reopening |
...
| Read-only history presentation for people and AI | Included in v0.7.0 as a deterministic local bundle: canonical JSON, Markdown, no-JavaScript HTML, manifest, checksums, and Synapse/GitHub target layouts; no upload or network access |
...
row describes library and schema surfaces included in the tagged v0.7.0
...
The tagged v0.7.0 source/workspace libraries include evaluation-only building
...
These capabilities are included in the tagged v0.7.0 source/workspace
...
distribution permission. The packaged v0.7.0 Creator Pilot and localhost UI
remain image-specific; their pending review authority is still same-process
and non-resumable.

The tagged v0.7.0 `synapse-local` binary includes browser import/review,
...
The v0.7.0 archive also provides the separate `synapse-present` companion. It reads
...
Separately, the tagged v0.7.0 source/workspace libraries include a versioned
...
- [`v0.7.0`](https://github.com/howlrs/synapsegit/releases/tag/v0.7.0) is a
...
- Release assets have SHA-256 checksums. The v0.7.0 archive also receives a
  GitHub build-provenance attestation.
- The object, archive, and OID formats remain draft and may change before a
  stable release.

See the [changelog](./CHANGELOG.md) and the
[v0.7.0 release notes](./docs/releases/v0.7.0.md) before evaluating the
