...
The current v0.8.0 preview is most useful to:
...
Open the exact `http://127.0.0.1:...` URL printed by the process. The v0.8.0
...
These v0.8.0 additions are included in the release binaries. See the [Creator workflow guide](docs/creator_workflow.md)
...
| Local browser interface | Read views, bounded three-file import, same-process `adopt` / `reject` / `defer`, read-only incomplete-session diagnostics, confirmed background `fsck`, and project-page archive controls. Behind `--archive-root`, it provides a bounded read-only archive listing (`GET /archives`) plus authenticated confirmed bounded archive export and empty-target restore APIs (`POST /archive-exports`, `POST /archive-restores`). Restore fixes the target to the open registered empty project and requires a listed slug, typed key, checkbox, confirmation, queued polling, and a visible creator-report equivalence reminder |
...
| Read-only history presentation for people and AI | Included in v0.8.0 as a deterministic local bundle: canonical JSON, Markdown, no-JavaScript HTML, manifest, checksums, and Synapse/GitHub target layouts; no upload or network access |
...
row describes library and schema surfaces included in the tagged v0.8.0
...
The tagged v0.8.0 source/workspace libraries include evaluation-only building
...
These capabilities are included in the tagged v0.8.0 source/workspace
...
distribution permission. The packaged v0.8.0 Creator Pilot and localhost UI
remain image-specific; their pending review authority is still same-process
and non-resumable.

The tagged v0.8.0 `synapse-local` binary includes browser import/review,
...
The v0.8.0 archive also provides the separate `synapse-present` companion. It reads
...
Separately, the tagged v0.8.0 source/workspace libraries include a versioned
...
App, and a hosted service remain distinct roadmap work; this source-only slice
does not complete [#17](https://github.com/howlrs/synapsegit/issues/17).
...
- [`v0.8.0`](https://github.com/howlrs/synapsegit/releases/tag/v0.8.0) is a
...
- Release assets have SHA-256 checksums. The v0.8.0 archive also receives a
  GitHub build-provenance attestation.
- The object, archive, and OID formats remain draft and may change before a
  stable release.

See the [changelog](./CHANGELOG.md) and the
[v0.8.0 release notes](./docs/releases/v0.8.0.md) before evaluating the
